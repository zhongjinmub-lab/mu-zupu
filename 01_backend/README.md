...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | /api/v1/settings/vector-search | 返回向量检索能力、隔离校验、检索链路和运维检查摘要 |

约束：
- 接口复用统一响应结构，不强制租户头；
- 响应只展示 pgvector/HNSW、1536 维 embedding、租户隔离、知识库隔离、检索日志和运维建议；
- 不返回数据库连接串、模型 API Key、原始向量内容或检索请求原文；
- 管理台设置页用中文卡片展示摘要，不新增黑色 JSON 原文区域。
